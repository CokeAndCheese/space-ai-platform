...
- Standard Model Package v1 machine implementation and joint technical acceptance are complete on the two task branches at Platform checkpoint `786e3f3` and Studio checkpoint `02b560a`. The repositories independently verified the identical golden ZIP SHA-256 `d0662cdfb95656def2d553a727ddeb88a3b946c9f2ecbefe2558fd83723423b0` and its SHA index; both full gates, the real-browser joint acceptance, and the final independent review passed with P0/P1/P2 all zero. This is a compatibility candidate awaiting explicit user milestone confirmation—not a published version, a merged result, or evidence that local `main` is compatible.
...
- The user has approved the process direction for a temporary, capability-declared Standard Model Package v2 whose topology capability is explicitly absent while geometry, Metadata `3.3-semantic`, SPACE/semantic data, final artifact reread, URI/SHA/revision/identity, and resource limits remain strict. This does not make v1 topology optional, admit `*-unvalidated` or legacy batch artifacts, permit empty/fake sidecars, or authorize embedded topology fallback.
- The v2 machine schema, exact fields, discovery, fixtures, diagnostics, and producer/consumer code are not yet jointly frozen or implemented. Until those gates pass, Platform must not claim v2 compatibility or acceptance. A future v2 session may expose only non-topology capabilities; topology-dependent routing/rendering/connector/blocker/AI/Quick Action must report the structured `TOPOLOGY_UNAVAILABLE` product semantic and must never be published as Graph ready.
