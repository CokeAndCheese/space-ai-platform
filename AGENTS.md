...
- The Platform v2 consumer candidate is complete on `codex/r2-standard-model-package` at parser `e5b6462`, lifecycle `2033525`, capability gate `7733284`, UI `c498452`, and fixture `f499da8`. It is not merged into local `main`, published, or milestone-approved. The candidate leaves v1/legacy results and `src/ssp/**` unchanged, publishes only Scene/Metadata ready with no graph, and capability-gates every topology-dependent routing/rendering/connector/blocker/AI/Quick Action path.
