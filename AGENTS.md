...
- Platform implemented the correction at `30e1b4e` and added null-special-floor Template query coverage at `d4475ba`. Exact `floorName`/`floorType` queries remain usable; numeric level queries and level-derived actions do not apply to a `null` level. All other v1/v2 identity, security, lifecycle, topology, and legacy-reader boundaries remain unchanged. Studio has provided authoritative v1/v2 same-byte fixtures and SHA indexes for this correction, but Platform mirror verification is still pending; do not claim Studio/Forge/Platform three-way compatibility, publication, merge, or milestone approval from the local checkpoints or unverified upstream evidence.
