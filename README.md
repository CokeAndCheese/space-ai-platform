...
- SSP 暴露 10 个 controller，共 81 个 controller 方法；`cameraController.controls` 是一个底层实例属性，不计入方法数。
- 顶层 context 工具 4 个（`setContext` / `getContext` / `hasContext` / `clearContext`），合计 85 个可调用函数。
...
- `topologyTool` 提供 23 个 API（legacy 图、v2 图与路线）；真实 Hospital/BIM 拓扑适配层尚未实现。
...
| `ssp.objectsTool` | 12 | 查询、`setHighlight`/`unHighlight`、可见性、楼层炸开/合拢及状态查询 |
...
`topologyTool` 是窄腰：只接收调用方显式提供的 graph、connector、blocker 数据，负责通用寻路和 Three.js 路线渲染；不解析 BIM、不推断 Hospital 连接关系、不调用其他 SSP controller。跨层连接必须由输入图显式提供，Hospital 拓扑适配器留在未来的应用/模板层。
...
5. 当前 worktree 含大量 `??` 未跟踪文件及 `AM` 文件。不要清理、重置或提交不属于当前任务的改动。
