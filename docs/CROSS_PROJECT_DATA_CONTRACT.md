...
- 生效日期：`2026-08-22`
- 状态：产品分离、契约治理和方案 B 已批准；原候选已完成双端机器实现与联合技术验收；2026-09-01 获批的 TOWER/ROOF nullable-level 修正已完成 P0 纠正、修订权威 fixture 同字节镜像、独立验签及 Platform 消费端真实浏览器验收，仍待用户兼容里程碑、各仓 local `main` 集成与三端联合收口
...
- 当前兼容结论：**Studio `f653264` 修订权威 v1/v2 fixture/index 已在 Platform `acbf1f4`/`135be84` 同字节镜像并独立验签，Platform 又在 `4ca8645bffc524d78e606eed6abfe24da608dda2` 验证基线完成修订 v1/v2 的 Studio ZIP 直导真实浏览器验收；这仅证明 Platform 任务分支消费端技术封版。用户兼容里程碑、其他产品证据、各仓 local `main` 集成与集成后门禁尚未完成，因此不得宣称 Studio/Forge/Platform 三端兼容、发布、合并或 `main` 已兼容**
...
- Studio 的 metadata-free 单层 Source GLB naming authority 是 `docs/AI_SOURCE_GLB_NAMING_SPEC_V1.md`，SHA-256 `33150f24fae21f7c2e15a9f804bb9067c9b75200cd089dce4703355c493fe679`。Forge 只在向 Studio 交付限定的单层 Source GLB、Bundle floor GLB 或 Studio Building Source 输出时引用/镜像它；这不替换、迁移或重定义 Forge 自身 SHA 前缀 `8bdf440a` 的 Floor/Building/Visualization 产品 authority。
- Studio Building Source GLB authority 是 `docs/AI_BUILDING_SOURCE_GLB_SPEC_V1.md`，SHA-256 `c41dedc540037cfadbae828e82da4f170a97732e7a96d2ff14744ef75e4446ae`；权威 vector index SHA-256 `085a3a08f54fb7f02ee9ef6e16242e47d7741bb5821fdc95869df10ef6cc4485`，`Building.glb` SHA-256 `86244b9a40f75e11397cf8ebc65dc0509ffb0337eece3c2a8ba2e1d32a04b864`。
- Studio Building Source Bundle 1.0 authority 是 `docs/AI_BUILDING_SOURCE_BUNDLE_SPEC_V1.md`，SHA-256 `32944bb4206d5e09f449d70fcfdd6cd67d510a9e1ed9a202b9b7442da24e1e41`；权威 fixture/index SHA-256 分别为 `9f75a9502847fd2caea7a5b29d0d44f3763ffbe34953ae79f7bf2c4d7c3f0964`、`40187738f3f31f79255b04bd73ec6cf66a3d3758c59dd32848ad0a83bdb89018`。
- Platform 不消费上述任何 authoring naming、Building Source GLB 或 Bundle 契约，不提供 reader 或拒绝后的 legacy fallback。Platform 只显式消费 Standard Model Package v1/v2；Forge 不消费 Package v1/v2。
...
Standard Model Package v1 原候选已完成双端技术验收；nullable 特殊层修订权威 fixture/index 已完成同字节镜像、独立验签与 Platform 消费端真实浏览器验收。在用户确认兼容里程碑、各仓 local `main` 集成并完成三端联合收口前：
...
6. Platform R1 的内部适配证据与本次 Studio 修订 ZIP → Platform v1/v2 直导浏览器验收均有效；后者只构成 Platform 消费端证据，不替代 Forge/Studio 生产端门禁或三端联合结论。
...
- 原候选的两端共同 fixture/validator、exporter/reader、交叉验证与浏览器验收均已完成；nullable 特殊层 P0 纠正后的权威共同 fixture/index 已完成 Platform 同字节镜像、独立验签与 v1/v2 真实浏览器消费端验收。用户里程碑、各仓 local `main` 集成和三端联合证据完成前不宣布兼容发布。

2026-08-22，两个项目总控已逐项确认 package identity、manifest 字段、URI/摘要、版本与资源限制、Metadata dual-read、embedded→sidecar 映射、diagnostics、fixture 权威索引和非规范审计附件，当时没有遗留设计分歧。2026-08-28，原候选完成机器实现、共同 fixture/validator、交叉验证、真实浏览器联合验收与独立复核。2026-09-01 用户批准 nullable 特殊层兼容修正；Studio 修订权威 checkpoint 为 `f653264`，Platform P0 与镜像 checkpoint 为 `acbf1f4`，真实 validated v2 查询 checkpoint 为 `135be84`。2026-09-04，Platform 在验证基线 `4ca8645bffc524d78e606eed6abfe24da608dda2` 完成修订 v1/v2 的 Studio ZIP 直导浏览器验收。下一门禁是各产品回报完整联合证据、分别取得用户兼容里程碑、集成各自 local `main` 并通过集成后门禁。
...
- Platform 修订浏览器链路 PASS：从 Studio 仓库权威路径直接导入 T/RF v1 ZIP 后得到 4 floors、Graph ready、T/RF endpoint 可用；同会话切换 v2 后得到 4 floors、Scene/Metadata ready、无 graph、`TOPOLOGY_UNAVAILABLE / PACKAGE_DECLARED_ABSENT` 和 Quick Action unavailable。精确“飞到塔楼/屋顶”分别只命中 `A_T`/`A_RF` 对象；控制台 error 为 0，仅有权威最小 GLB 的 POSITION min/max warning。完整记录见 [`STANDARD_MODEL_PACKAGE_TRRF_PLATFORM_ACCEPTANCE.md`](./STANDARD_MODEL_PACKAGE_TRRF_PLATFORM_ACCEPTANCE.md)。
...
- Platform 的修订 fixture、自动矩阵和真实浏览器验收不替代其他产品证据、用户兼容里程碑、各仓 local `main` 集成或三端联合收口。
...
- 原 v1/v2 golden SHA 只证明修正前候选；nullable 特殊层的修订权威 fixture/index 已在 `acbf1f4`/`135be84` 完成 Platform 同字节镜像、P0 回归与查询验证，并在验证基线 `4ca8645bffc524d78e606eed6abfe24da608dda2` 通过 Platform 真实浏览器验收。其他产品联合证据、用户兼容里程碑和各仓 local `main` 集成仍未完成，因此不得宣称 Studio/Forge/Platform 三端兼容。
...
| 2026-09-01 | 批准 v1/v2 Package 3.3 TOWER/ROOF nullable-level 兼容修正；不新增 schema/version/elevation/order/topology，不推导 level | P0 order 推测、修订权威 fixture、自动矩阵及 Platform v1/v2 浏览器验收通过；用户里程碑、local `main` 与三端联合收口待完成 | 无人员变动 |
