...
- 生效日期：`2026-08-22`
- 状态：产品分离、契约治理和方案 B 已批准；原候选已完成双端机器实现与联合技术验收；2026-09-01 获批的 TOWER/ROOF nullable-level 修正已完成 Platform 实现、权威 fixture 同字节镜像与独立验签，仍待新的联合真实浏览器验收和用户兼容里程碑批准
...
- 当前兼容结论：**nullable 特殊层权威 v1/v2 fixture/index 已在 Platform 同字节镜像并独立验签；但尚未完成本修正的联合真实浏览器验收，也未获用户兼容里程碑批准，因此不得宣称 Studio/Forge/Platform 三端兼容、发布、合并或 `main` 已兼容**
...
Standard Model Package v1 原候选已完成双端技术验收；nullable 特殊层修正的权威 fixture/index 已完成同字节镜像与独立验签，仍待新的联合真实浏览器验收。在用户确认兼容里程碑前：
...
- 原候选的两端共同 fixture/validator、exporter/reader、交叉验证与浏览器验收均已完成；nullable 特殊层修正的权威共同 fixture/index 已完成 Platform 同字节镜像与独立验签，但新的联合真实浏览器验收尚未执行，用户里程碑确认前不宣布兼容发布。

2026-08-22，两个项目总控已逐项确认 package identity、manifest 字段、URI/摘要、版本与资源限制、Metadata dual-read、embedded→sidecar 映射、diagnostics、fixture 权威索引和非规范审计附件，当时没有遗留设计分歧。2026-08-28，原候选完成机器实现、共同 fixture/validator、交叉验证、真实浏览器联合验收与独立复核。2026-09-01 用户批准 nullable 特殊层兼容修正；Platform checkpoints 为 `30e1b4e`（validator/Metadata/lifecycle）、`d4475ba`（Template 查询专项）与 `35161cc`（权威 fixture 镜像验签）。下一门禁是 nullable 特殊层新的联合真实浏览器验收和用户兼容里程碑批准。
...
- nullable 特殊层镜像 checkpoint `35161cc` 保存 `src/test/package/fixtures/building-source-profile-v1.1/{sha256.json,A-standard-model-package-v1.zip,A-standard-model-package-v2.zip}`，与 Studio 原件 `cmp=0`。authority document SHA 为 `7df85d3992559ba299b08ce8e55917c732291a519175de6c71c4b422eb128f0f`，index SHA 为 `f72befd8dc538095fdca43d5979c968b57b87d1c8650a81ecb8a337405a80ef1`，v1 ZIP SHA 为 `b2cc39d73504f2f8ed2535305785f9a32e3b93493eebbb52737ce90b404b2be4`，v2 ZIP SHA 为 `a07fe215f0c878d407378d033328f8ad7d3602b94c89701c2266896ac41ef77c`；index、两个 ZIP、全部 entry SHA/length 已自动独立校验。
...
- 以上只关闭 nullable 特殊层 fixture 镜像和自动验签，不替代新的联合真实浏览器验收或用户兼容里程碑批准。
...
- v1 继续强制 strict sidecar topology；除用户批准并由 v1/v2 同步采用的 TOWER/ROOF nullable-level 显式接受值域外，v1 identity/topology、既有 fixture 结果与旧 v3.1 reader 不变；修正后的共同 fixture 已由 Studio 提供，并在 Platform `35161cc` 完成同字节镜像与独立验签。
...
- 原 v1/v2 golden SHA 只证明修正前候选；nullable 特殊层的新权威 fixture/index 已在 `35161cc` 完成 Platform 同字节镜像与独立验签。新的联合真实浏览器验收和用户兼容里程碑仍未完成，因此不得宣称 Studio/Forge/Platform 三端兼容。
...
| 2026-09-01 | 批准 v1/v2 Package 3.3 TOWER/ROOF nullable-level 兼容修正；不新增 schema/version/elevation/order/topology，不推导 level | Platform 实现与权威 fixture 同字节镜像验签完成；新联合浏览器/用户里程碑待完成 | 无人员变动 |
