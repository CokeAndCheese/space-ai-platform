# 交接提示词 — Space AI Platform（2026-08-10）
...
| 文档 | 用途 |
|---|---|
| `README.md` / `PROJECT_SUMMARY.md` / 本文 | 2026-08-10 当前状态与交接入口 |
...
- 10 个 controller，81 个 controller 方法；`cameraController.controls` 是实例属性。
- 顶层还有 4 个 context 函数：`setContext`、`getContext`、`hasContext`、`clearContext`；合计 85 个可调用函数。
...
- 真实 Hospital/BIM topology 适配层尚未实现。topology 核心只处理调用方显式提供的 graph、connector、blocker 和路线，不解析模型、不推断连接关系、不调用其他 controller。
...
1. 在应用/模板层实现 Hospital/BIM metadata 到显式 topology graph 的适配器。
2. 按需求评审新增 AI-enabled 模板，保持危险能力 opt-in。
3. 继续按实际数据补齐性能和 metadata 观测。
...
- 不清理、重置或提交并行任务的文件。当前 worktree 有大量 `??` 未跟踪和 `AM` 文件，属于高风险共享状态。
...
**TL;DR**：先按首读顺序和命令确认 79/0/4/9 基线；AI 只能走模板；topology 只处理显式通用图，Hospital 适配器仍待实现；共享 worktree 不清理、不重置、不提交。
