...
> 日期：2026-08-22
>
> 状态：方案 B 已获用户批准；原候选已完成双端机器实现与联合技术验收；2026-09-01 获批的 TOWER/ROOF nullable-level 修正已完成 order 推测 P0 纠正、修订权威 fixture 同字节镜像、自动矩阵与 Platform 消费端真实浏览器验收；用户兼容里程碑、其他产品证据和各仓 local `main` 集成仍待完成，尚未发布或进入 `main` 兼容基线
>
...
原候选的共同 fixtures、producer validator、consumer validator、双端全门禁、真实浏览器端到端验收和最终独立复核均已通过。2026-09-01 获批的 TOWER/ROOF nullable-level 修正已完成修订权威 v1 fixture/index 的 Platform 同字节镜像、P0 回归、独立验签和 Studio ZIP → Platform 真实浏览器验收。该证据只关闭 Platform 消费端任务分支门禁；用户确认、其他产品证据和各仓 local `main` 集成前仍只能称为“兼容候选”，不得宣称 Studio/Forge/Platform 三端兼容、发布、合并或 `main` 已兼容。
...
- Platform 在验证基线 `4ca8645bffc524d78e606eed6abfe24da608dda2` 从 Studio 权威仓库路径直接导入修订 v1 ZIP：4 floors、revision `482a129ffeef87de842d86ecb62e9e2d2f693ebcc0ae194543ab6e82c7f142bb`、Graph ready、`A_T`/`A_RF` endpoint 可用；精确塔楼/屋顶查询只命中对应对象，控制台 error 为 0。完整记录见 [`STANDARD_MODEL_PACKAGE_TRRF_PLATFORM_ACCEPTANCE.md`](./STANDARD_MODEL_PACKAGE_TRRF_PLATFORM_ACCEPTANCE.md)。
- 旧 `35161cc`/`dbceba4` 及旧 authority/index/v1 ZIP/revision SHA `7df85d3992559ba299b08ce8e55917c732291a519175de6c71c4b422eb128f0f`、`f72befd8dc538095fdca43d5979c968b57b87d1c8650a81ecb8a337405a80ef1`、`b2cc39d73504f2f8ed2535305785f9a32e3b93493eebbb52737ce90b404b2be4`、`dddc3c9f5c3ee48d3e8123d8ae1d9b5ea591d1eced7ebf3bfcda8ce6e4b4c147` 已 superseded，仅作历史证据。Platform 修订浏览器证据不替代其他产品证据、用户兼容里程碑或各仓 local `main` 集成。
...
- 原候选的双端实现、共享 fixture、自动验证、浏览器加载和独立 QA 已通过；2026-09-01 nullable 特殊层修正也已完成 P0 纠正、修订权威 v1 fixture 同字节镜像、自动矩阵与 Platform 消费端真实浏览器验收。仍须收齐其他产品联合证据、获得用户兼容里程碑批准，并在另行授权后完成各仓 local `main` 集成与集成后门禁；此前不得申请三端兼容、发布、合入或备份结论。
...
6. **已完成（Platform 消费端）**：在 `4ca8645bffc524d78e606eed6abfe24da608dda2` 验证基线完成 nullable 特殊层修订 v1/v2 的 Studio ZIP 直导真实浏览器验收，P0/P1/P2 为 0。
7. **当前门禁**：收齐 Forge/Studio/Platform 联合证据并向用户提交各产品兼容里程碑确认；确认后再执行另行获准的本地合入、集成后验证与手动备份流程。
